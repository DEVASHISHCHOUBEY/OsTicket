...
class GetHelpTopic(viewsets.ViewSet):
    ...
    def create(self,request):
        ...
        print("=============================",topic)
        help_obj  = help_topic.objects.filter(topic = topic)
...
class GetTicketHistory(viewsets.ViewSet):
    """API to get history of ticket created by stakeholder """
    
    def create(self,request):
        request_data = request.data
        obj_list = ticket.objects.filter(user_id = 15)
        response_dt = []
        for obj in obj_list:
            response_dt.append(GetStatusOfListOfTicketId.get_os_ticket_status(obj))
        return Response({"st":StatusCode.OK,"dt":response_dt})
    
class GetTicketStatus(viewsets.ViewSet):
    """ API to get Get Ticket Status by Ticket Id """
    
    def create(self,request):
        request_data = request.data
        ticket_obj = ticket.objects.get(ticket_id=request_data['ticket'])
        response_dt = GetStatusOfListOfTicketId.get_os_ticket_status(ticket_obj)
        return Response({"st":StatusCode.OK,"dt":response_dt})
